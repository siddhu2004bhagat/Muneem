...
>(({ className, children, ...props }, ref) => (
  <DrawerPortal>
    ...
    <DrawerPrimitive.Content
      ref={ref}
      className={cn(
        "fixed left-0 right-0 bottom-0 z-50 flex h-auto flex-col rounded-t-[10px] border-t border-l border-r bg-background shadow-lg",
        className,
      )}
      {...props}
    >
      <div className="mx-auto mt-2 h-1 w-12 rounded-full bg-muted/60" />
      {children}
    </DrawerPrimitive.Content>
  </DrawerPortal>
));
